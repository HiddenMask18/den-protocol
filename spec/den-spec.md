...
**Master secret** — the Creator-held cryptographic secret from which content encryption keys are derived. The master secret is never stored in plaintext on any instance. It is stored as an ECIES-encrypted operational blob (decryptable by the instance for key delivery) and a separate portability blob (encrypted to the Creator's wallet, for migration and recovery). Defined in Section 4.1.
...
**V1 security model:** The master secret is never stored in plaintext on the instance. The Creator's client generates it and encrypts it before uploading. A legally compelled hoster obtains ciphertext only in normal operation; master secrets are not extractable without the instance-derived decryption key. The primary defense against legal compulsion is portability and key rotation: a Creator can migrate to a different-jurisdiction instance within hours, and rotating the master secret makes the seized instance's operational blob useless for new content — old ciphertext remains encrypted under the old keys.

**Operational blob** — ECIES ciphertext of the `master_secret` (32 bytes), encrypted to the instance's per-creator derived public key. The instance decrypts this on demand at key delivery time. The operational blob MUST be present before key delivery is possible. The instance derives the per-creator key deterministically from a single instance-wide master key using HKDF, so no per-creator keys are stored at rest.

**Portability blob** — ECIES ciphertext of the `master_secret`, encrypted to the Creator's primary wallet public key. The instance cannot decrypt this. The Creator decrypts it using their wallet private key for migration and recovery (Section 8.1).

Both blobs MUST be configured by the Creator before key delivery is possible. The instance MUST verify that the operational blob decrypts to exactly 32 bytes. The `master_secret` MUST be zeroed from memory immediately after content key derivation. The instance MUST NOT persist any decrypted key material to durable storage.

**Design note — threshold split (V2 path):** An earlier design used a two-party XOR split (`instance_share` held by the instance, `creator_share` held by a per-Creator oracle service) so that a compelled hoster could not derive keys without additionally compelling the Creator's oracle. This design was removed from V1 because the separation relies on organisational independence, not cryptographic enforcement — nothing prevents the same operator from running both the instance and the oracle, and there is no way to detect or prevent this. Creator liveness dependency also creates a single point of failure: if the oracle is offline or deleted, all subscribers lose decryption access immediately. If a threshold decryption scheme backed by a decentralised independent custodian network becomes production-ready (e.g. a Lit Protocol or similar approach where access policies are enforced cryptographically rather than operationally), a threshold model should be revisited for V2.
