...
**Master secret** — the Creator-held cryptographic secret from which content encryption keys are derived. The master secret is never stored whole on any instance. It is split into two independent halves (threshold split): `instance_share` held by the instance in an encrypted operational blob, and `creator_share` held by the Creator's independently-operated oracle. Key delivery requires both. The Creator retains the full master secret in a portability blob (encrypted to their wallet) for migration and recovery. Defined in Section 4.1.
...
When a Creator's active wallet changes via rotation, the primary portability blob MUST be re-encrypted to the new wallet public key. The operational blob is encrypted to the instance's derived key and is unaffected by wallet rotation. If emergency wallets are registered, their emergency portability blobs MUST also be re-encrypted (using the current master secret, which remains unchanged unless a full key rotation is performed simultaneously). Re-encryption of portability blobs requires access to the current master secret via the current active wallet or any registered emergency wallet.

**Access grant re-publication after rotation:** Access grant declarations are signed by the Creator's wallet at authorship time. After wallet rotation, all existing grants' signatures verify against the old wallet, not the new one. A receiving instance verifying grant signatures against the current primary wallet will reject these grants. The Creator MUST re-publish all access grants — signed by the new wallet — after rotation completes, before Subscriber key delivery will succeed. Rotation without grant re-publication creates a window where subscription access fails; Creators SHOULD minimise this window by re-publishing grants promptly. See Section 4.7 for full key rotation mechanics.
...
**Master secret:** Each Creator holds a master secret — a cryptographic secret from which all content keys for that Creator are derived. The master secret is generated by the Creator's client at account creation.

**Threshold key split:** The master secret is never stored whole on the instance. The Creator's client splits it into two independent halves using a one-time pad:

```
instance_share = 32-byte random
creator_share  = master_secret XOR instance_share
```

These halves are held by two independent parties: the instance holds `instance_share` (encrypted as the operational blob), and the Creator holds `creator_share` via an independently-operated oracle service. Neither party can derive content keys alone. Key delivery requires both parties to cooperate.

This split eliminates the single-party capture vector: a compelled instance hoster can produce `instance_share` for every Creator on the instance — but `instance_share` is information-theoretically indistinguishable from random noise without the corresponding `creator_share`. Decrypting any Creator's content requires a separate action against that specific Creator's oracle. To decrypt N Creators' content requires N independent actions.

**Operational blob** — ECIES ciphertext of `instance_share` (32 bytes), encrypted to the instance's per-creator derived public key. The instance decrypts this on demand at key delivery time to obtain `instance_share`. The operational blob MUST be present before key delivery is possible. The instance derives the per-creator key deterministically from a single instance-wide master key using HKDF, so no per-creator keys are stored at rest.

**Portability blob** — ECIES ciphertext of the full `master_secret`, encrypted to the Creator's primary wallet public key. The instance cannot decrypt this. The Creator decrypts it using their wallet private key for migration and recovery (Section 8.1). From `master_secret`, the Creator can always re-derive both `instance_share` and `creator_share` for any instance they migrate to.

**Creator oracle** — a minimal HTTP service operated by the Creator, independent of the instance. The oracle holds `creator_share` as a secret and returns it only to requests that can demonstrate valid on-chain entitlement. The oracle MUST verify subscription or purchase state directly from the chain — it MUST NOT accept the instance's claims about entitlement without independent verification. The Creator's oracle URL is registered by the Creator and stored alongside the operational blob. The Creator MAY take the oracle offline at any time; doing so immediately prevents new key derivations even if the hoster has full access to all instance data.

Both blobs and the oracle URL MUST be configured by the Creator before key delivery is possible. The instance MUST verify that the operational blob decrypts to exactly 32 bytes. The instance share MUST be zeroed from memory immediately after key reconstruction. The instance MUST NOT persist any reconstructed key material to durable storage.

When an emergency wallet is registered, an additional emergency portability blob MUST be created, encrypted to the emergency wallet's public key. The emergency wallet can independently decrypt this blob and re-derive both shares for rotation or migration (Section 4.7). Subsequent wallet rotations MUST update all portability blobs accordingly (Section 4.7, Section 2.5.7).
...
- Encrypted master secret blobs: operational blob (accessible only by the instance, for key delivery) and portability blob (accessible only by the Creator via their wallet, for migration and recovery). If the Creator has registered emergency wallets, an emergency portability blob per registered emergency wallet MUST also be stored.
- Content metadata: unique content fingerprint (hash), tier assignment, timestamp, public content warnings
- Subscriber subscription state: wallet addresses, tier, active expiry — this is public on-chain state that MUST be mirrored locally. The local mirror is updated on every successful key delivery and is authoritative for portability export (Section 8.1). On-chain state remains the access authority; the local mirror is for operational and export use only.
...
- Plaintext master secret at any point — the operational blob MUST be zeroed immediately after key derivation; the portability blobs are stored as opaque ciphertext the instance cannot read
...
**Rate design intent:** Storage and bandwidth rates are governance parameters calibrated to produce a stable creator effective fee across all instance sizes above the micro bracket. The calibration target is approximately 1.8% effective creator fee after surplus return — well below the stated 2.5% fee, and well below any incumbent subscription platform. Infrastructure cost is the floor, not the calibration anchor: rates must cover actual infrastructure cost at minimum, but are set above that floor to produce the target effective fee and a viable hoster income at each bracket.

**Fee stability as anti-centralization mechanism:** A rate table that produces declining creator effective fee at larger instance sizes creates a direct financial incentive for creators to migrate toward the largest available instance. Because creator portability is a protocol guarantee, creators can and will act on this incentive. The result is ecosystem centralization driven by fee economics rather than lock-in — functionally identical to platform capture by a different mechanism. Calibrating rates to maintain stable creator effective fee across brackets eliminates this incentive: a creator's fee experience is approximately the same regardless of instance size, so instance selection is driven by content policy fit, community, and operator trust — not price.

**Hoster income scales with absolute volume, not per-unit rate.** A larger instance earns more than a smaller one because it serves more storage and bandwidth, not because it sits in a higher-rate bracket. The compound return for hosters comes from growing their instance.

**The large bracket rate inversion:** The large instance bracket carries higher per-unit rates than the medium bracket. This is intentional and is the direct consequence of fee stability targeting. At large scale, the fee pool is substantially larger relative to resource consumption — applying declining rates to a large pool produces a small hoster claim and routes most of the fee pool back to creators as surplus, collapsing the effective creator fee below 0.25%. The higher large bracket rate keeps the hoster claim proportional to the fee pool, maintaining effective creator fee stability. Governance MUST NOT reduce large bracket rates below medium bracket levels without first modelling the effective creator fee impact across instance sizes — doing so recreates the fee-driven centralization incentive this calibration is designed to prevent.

**Cap mechanism:** The hoster claim is capped at the available fee pool: see formula in Section 13.2. At micro scale, the formula result exceeds the fee pool — the cap activates, the hoster receives the entire fee pool, and no creator surplus returns. Creators at micro scale instances pay the full stated 2.5% with no rebate. As the instance grows above approximately 80 creators at typical subscription prices, the formula result drops below the fee pool, the cap deactivates, and surplus begins returning to creators. This transition is visible on-chain and is the compound return signal for hosters: the larger the instance, the more creators receive back as surplus, making a growing instance more attractive to creators without any further intervention.
...
`hoster_claim = min((storage_GB × bracket_storage_rate) + (bandwidth_GB × bracket_bandwidth_rate), fee_pool)`

The cap ensures the hoster claim never exceeds the fee pool available in that creator's escrow at the time of settlement. At micro scale instances the formula result typically exceeds the fee pool — the cap activates, the hoster receives the full fee pool, and surplus return to the creator is zero. At small scale and above, the formula result is below the fee pool — the cap is inactive and surplus returns to the creator. The cap is a per-creator-escrow calculation applied at each settlement interval, not an aggregate instance-level cap.
...
|-----------|-------------|---------------|
| `storage_rate` | Named alias for the medium bracket storage rate within `progressive_rate_parameters`. Provided for reference in settlement calculations and spec prose. `progressive_rate_parameters` is authoritative — this value MUST reflect the medium bracket rate in that table at all times. Adjusted only as a consequence of adjusting `progressive_rate_parameters`, not independently | $0.30/GB |
| `bandwidth_rate` | Named alias for the medium bracket bandwidth rate within `progressive_rate_parameters`. Same aliasing constraint as `storage_rate`. | $0.40/GB |
...
| `progressive_rate_parameters` | Full rate table mapping instance size brackets to storage and bandwidth rates per GB. Rates are calibrated to maintain a stable creator effective fee of approximately 1.8% across all brackets above micro. The large bracket carries higher per-unit rates than the medium bracket — this is intentional; see Section 7.2 rate design intent for the centralization rationale. Governance MUST NOT reduce large bracket rates toward medium bracket levels without modelling effective creator fee impact. Instance size computed as `creator_count + subscription_relationship_count`; same-instance relationships excluded per Section 9.3. Proposed launch values: Micro (<80 creators) — storage $0.60/GB, bandwidth $0.80/GB. Small (80–200) — storage $0.45/GB, bandwidth $0.60/GB. Medium (200–500) — storage $0.30/GB, bandwidth $0.40/GB. Large (500+) — storage $0.46/GB, bandwidth $0.69/GB. All values recalibrated by governance as operational data accumulates; the ~1.8% effective fee target is the calibration constraint, not any specific rate value. | See proposed launch values in description |
