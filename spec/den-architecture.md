...
- Identity contract as the stable on-chain identifier — per-participant proxy contract on the canonical identity chain (Base); wallet address held inside it is the active key, not the identifier itself
- Canonical identity chain — Base (EVM L2); TRON and Solana are payment rails only
- Authentication standard: [EIP-4361 (Sign-In with Ethereum)](https://eips.ethereum.org/EIPS/eip-4361) for EVM-compatible chains, [Wallet Adapter](https://github.com/anza-xyz/wallet-adapter) for Solana, TronWeb for TRON
...
- Emergency wallet — optional secondary key registered against the identity contract; enables identity recovery without protocol-held infrastructure
- Wallet rotation — dual-sig for clean case; time-delayed unilateral for compromise case; any registered wallet can cancel during delay window
- Wallet revocation — any registered wallet can announce removal of another; same time-delay mechanism
- MFA framing — client-layer UX protection only; not a protocol-level security guarantee
- Client convenience layers — password and email recovery defined as client-layer concerns only; auditability requirement stated; wallet key material never leaves device
...
**Design rationale — identity contract over raw wallet address:** Subscriptions and purchase state addressed directly to a wallet address require all downstream state to be migrated when a wallet rotates. At meaningful subscriber and purchase counts this is prohibitively expensive — hundreds or thousands of on-chain operations per rotation. The identity contract provides a stable address that never changes. Rotation updates one record inside the contract. All downstream state continues to resolve correctly without migration. This is the standard solution to the stable-identifier-over-mutable-key problem and does not introduce new trust assumptions.

**Design rationale — proxy pattern for identity contract:** An immutable identity contract cannot be patched if a vulnerability is discovered. A centrally upgradeable contract reintroduces a kill switch at the infrastructure layer — whoever holds upgrade keys can modify participant identity contracts. The proxy pattern with participant-held upgrade keys is the correct middle position: each participant can upgrade their own contract logic when the protocol publishes a new reference implementation; no participant can upgrade any other participant's contract; no central authority exists to compel upgrades. Non-upgraded participants run older logic indefinitely — this is accepted as the cost of decentralization and stated honestly.

**Design rationale — EVM as canonical identity chain:** EVM smart contract tooling — Solidity, Hardhat, Foundry, the audit ecosystem, the deployed contract pattern library — is by a significant margin the most mature available. Base is the chosen primary L2: low fees, EVM-native, established infrastructure. The identity contract model is well-validated on EVM through projects like ENS and Lens Protocol. Genuine multi-chain identity parity — a Solana-native participant with no EVM requirement — requires solving the cross-chain state resolution problem in a trustless way that does not currently have a clean answer at this protocol's V1 scope. The honest V1 position is: EVM-canonical for identity, payment-rail for everything else. This is stated explicitly so implementations don't accidentally imply parity that doesn't exist. Revisable by governance as cross-chain identity infrastructure matures.

**Design rationale — emergency wallet as participant-held mitigation:** The wallet recovery infrastructure rejection (Appendix A) is correct and stands. Any recovery mechanism the protocol holds is a compellable target. The emergency wallet is architecturally different: the protocol holds nothing; the participant registers their own second wallet as an additional authorized key; the instance stores one master secret blob encrypted to multiple participant-held public keys. No new compellable target exists. The participant's own emergency wallet is an extension of their own custody model, not a dependency on protocol infrastructure. The friction of cold storage custody for the emergency wallet is the load-bearing element — it is what makes the emergency wallet a meaningful second factor rather than an easily-captured backup.

**Design rationale — time-delayed unilateral rotation:** Requiring dual-signature for all rotations means a participant who loses their primary wallet also loses the ability to rotate their identity. This is an unacceptable outcome for the compromise scenario the emergency wallet is designed to address. Time-delayed unilateral rotation solves this: a registered wallet announces rotation, the delay window gives any other registered wallet time to cancel if the announcement is unauthorized, and the rotation completes automatically if legitimate. The delay is the security mechanism — it converts rotation from an instant operation into a visible process with a cancellation window. The delay duration is a governance parameter reflecting the tradeoff between security window length and recovery friction.

**Design rationale — cancellation and revocation symmetry:** Any registered wallet can cancel a pending rotation or revocation during the delay window. This is the only logical position — if only the primary wallet could cancel, a stolen primary wallet could prevent the emergency wallet from completing a legitimate rotation. If only the emergency wallet could cancel, a compromised emergency wallet could block legitimate rotation. Symmetry means either wallet can defend the identity against the other being compromised, up to the limit of both wallets being compromised simultaneously, which is the accepted total-loss scenario.

**Design rationale — MFA as client-layer framing:** MFA in the DEN context protects against casual unauthorized access to a device or client application. It does not raise the security floor of the protocol-level identity model. The seed phrase holder has full identity access regardless of client-layer MFA. Framing MFA as a protocol-level security guarantee would be dishonest and would give participants false confidence in what it actually protects. Client implementations should offer MFA as a usability and device-security feature, not as a substitute for proper seed phrase custody.

**Design rationale — client convenience layer auditability requirement:** Email recovery and password recovery are genuinely useful for the majority of participants who will use them and who would otherwise face prohibitive friction. They are acceptable client-layer features with one non-negotiable constraint: wallet key material must never leave the participant's device. The only way this constraint is verifiable is open source code. The auditability requirement is therefore not optional for client implementations offering these features — it is the mechanism by which the community can confirm the constraint is met. A client offering email recovery that is not open source cannot be trusted, regardless of what it claims.
...
**Design rationale — protocol fee applies to shop items and packs:** The protocol fee applies to shop item and pack purchases at the same rate as subscription payments. The logic is identical — hosters store and serve that content, the per-creator escrow model needs funding from the transactions that generate access load. The one structural difference is that subscriptions are recurring (fee collected every renewal period) while shop item purchases are one-time (fee collected once regardless of subsequent access frequency). The resource formula's bandwidth rate handles ongoing access cost independently, so there is no misalignment — the escrow contribution from a purchase is proportional to the transaction value, and ongoing serving cost is compensated through the standard bandwidth rate.

**Design rationale — per-creator escrow over central treasury:** The fee pool must have a home between collection and hoster claim. A central treasury reintroduces a single point of control — whoever holds or manages the treasury has leverage over the network. Per-creator escrows are structurally different: each creator's fee pool is their own on-chain account, the hoster claims from it based on the resource formula, and any surplus returns to that creator without passing through any intermediary. The no-central-treasury principle is preserved. The creator-sustainability principle is reinforced — surplus from the creator's own escrow returns to them directly.

**Design rationale — 2.5% protocol fee:** 2.5% is creator-favorable compared to any incumbent subscription platform (Patreon: 5-12%, OnlyFans: 20%). It is sustainable at small instance scale — the resource formula generates meaningful hoster margin above infrastructure cost at calibrated rates. It is evaluated against creator sustainability as the primary metric. Governance may adjust it, but downward adjustment is explicitly preferred when the ecosystem generates consistent surplus above hoster needs.

**Design rationale — progressive rate intent:** Infrastructure overhead per creator is higher at small instance scale — fixed costs are spread across fewer creators. Calibrating rates to provide higher effective multipliers for smaller instances corrects this imbalance without explicit per-size pricing tiers. The intent is expressed as a governance calibration principle rather than a fixed formula, allowing the rate table to be updated as operational data accumulates without requiring spec amendment.

**Design rationale — instance size formula:** Instance size for bracket assignment is computed as `creator_count + subscription_relationship_count`, where subscription relationships are counted per relationship rather than per unique subscriber wallet. A subscriber active on three creators within the same instance counts as three subscription relationships — this more accurately reflects actual resource load (bandwidth, access requests) than deduplicating unique wallets, and it is directly derivable from on-chain state without self-reporting. Same-instance relationships are excluded by the same logic as the self-transaction exclusion in Section 9.3. The formula feeds into governance-set brackets rather than a continuous rate function — brackets are simpler to implement in smart contracts, easier to audit, and easier for governance to reason about when adjusting. Formula weights and bracket thresholds are governance parameters recalibrated as operational data accumulates; the correct weight for subscription relationships relative to creator count is an empirical question best answered once instances are running.

**Known limitation — fee pool and infrastructure cost decorrelation:** The protocol fee is a percentage of subscription revenue; infrastructure cost is driven primarily by subscriber count and bandwidth consumption. These decorrelate at extremes — a creator with many subscribers at a low price generates high infrastructure cost and low fee pool; a creator with few subscribers at a high price generates the inverse. This is a known approximation acceptable for launch. Governance should monitor for systematic misalignment and has the correction mechanism available through rate and fee adjustment.
...
**Design rationale — key rotation resumability and blob replacement timing:** Re-encryption is a potentially long-running operation over large content libraries. Requiring a full restart on any interruption imposes unbounded cost on Creators with large back-catalogues and creates a failure mode where re-encryption can never complete on an unreliable connection. Resumability from on-chain state costs nothing architecturally — the on-chain fingerprint record is already the authoritative source of which content exists at which version, and a resuming client can diff expected new fingerprints against current on-chain state to determine exactly where the interruption occurred. No separate progress tracking is needed and none should be introduced. The blob replacement timing constraint follows directly from the re-encryption model: the old master secret must remain the valid decryption source until all tiers have completed, otherwise a partially-rotated library has content that is inaccessible under both old and new keys simultaneously. Blob replacement as the final step is the transactional integrity guarantee for the rotation operation as a whole.
...
**Design rationale — access grant declaration signature requirement:** Access grant declarations govern which derivation paths a subscriber or buyer is entitled to — they sit directly in the access control model. An instance that accepts unverified declarations during migration could be presented with silently modified declarations granting broader access than the Creator originally authorised. An instance that does not verify declarations at access request time is vulnerable to a compromised or malicious instance swapping declarations post-ingestion without detection. Requiring creator wallet signatures on all declarations and verifying them at both ingestion and access request time closes both vectors with the same mechanism already used throughout the protocol for authentication. The verification cost is identical to the wallet signature verification already required at every access request — no new cryptographic infrastructure is introduced.
...
**Design rationale — mandatory IPFS pinning:** The portability guarantee is a fixed spec value. A Creator whose content is not pinned to IPFS is dependent on the instance operator maintaining their local copy. A hostile operator can delete the local copy before the Creator completes migration, leaving content inaccessible during the migration window even if the Creator holds all their portable data. IPFS pinning closes this gap: the protocol requires hosters to maintain an IPFS-pinned copy of all active content tiers, so a hostile operator who deletes locally cannot delete the IPFS object. Pinning is part of what "storing content correctly" means under this protocol and is compensated within the resource formula rates.

**Design rationale — storage compensation threshold:** The threshold is the arithmetic consequence of the escrow model, not a separate policy decision. A Creator with no active subscribers generates no subscription revenue, therefore no fee pool, therefore no compensable claim for the hoster. The threshold makes this explicit and prevents hosters from accumulating phantom compensation claims for abandoned content. The migration window exclusion prevents penalizing Creators for the temporary subscriber gap that occurs during legitimate instance migration. The threshold and the passive deletion trigger are deliberately separate mechanisms — compensation stops before deletion begins, creating a bounded grace window.

**Design rationale — compliance registry as governance function:** Technical compliance with the spec is auditable — a compliant instance stores only ciphertext, follows the removal process, applies standards uniformly, participates in batch settlement. These are verifiable behaviors. The compliance registry records the governance community's findings on these verifiable behaviors. It is not an endorsement or recommendation system. The distinction matters: whoever controls an endorsement list holds soft power over which instances get traffic. The founding maintainer controlling that list would be soft power accumulation by another name. Separating compliance (protocol function, governance-operated) from recommendation (community function, community-operated) means the founding maintainer has a defined technical accountability role and no special role in community recommendation. Community instance directories maintained independently are the correct layer for recommendation.

**Design rationale — bandwidth verification acknowledged limitation:** Storage overclaim is detectable on-chain because content fingerprints are recorded at upload. Bandwidth overclaim has no equivalent on-chain record — the protocol cannot verify how much data was actually served without introducing monitoring infrastructure that would itself be a privacy and centralization concern. The declared-plus-auditable model is an honest first approximation: declarations are on-chain and visible, implausible claims are detectable by community analysis, and governance handles substantiated findings. A more rigorous verification mechanism is a V2 scope item requiring careful design to avoid introducing the monitoring infrastructure problem.

**Open questions:** None. Batch settlement mechanics and transaction fee optimization are technical implementation decisions deferred to the smart contract design phase.
...
**Design rationale — maintainer-hoster recusal norm:** The founding maintainer and any maintainer who also operates an instance has a direct financial interest in governance parameters that affect hoster compensation — specifically `storage_rate`, `bandwidth_rate`, `progressive_rate_parameters`, and `protocol_fee_pct`. This conflict of interest is real regardless of intent. The correct response is transparency and declared recusal, not prohibition: during the bootstrap phase there may not be enough non-conflicted maintainers to reach consensus without conflicted voices, so hard exclusion is impractical. The norm is stated in CONTRIBUTING.md rather than encoded as a protocol rule because it governs governance conduct, not protocol mechanics. The norm: declare the interest, recuse where the community's trust in the outcome requires it, and never participate undeclared.

**Design rationale — founding instance equal treatment:** The founding instance operates under the same compensation formula as every other instance. There is no founding instance premium, no special rate, no bootstrap compensation beyond the initial community-funded runway that ends when the instance becomes operationally self-sustaining. This is stated explicitly before the formula is finalized to remove any ambiguity about whether the founding maintainer's position as likely operator of the first prominent instance influenced the formula design. The founding instance designation itself is sunset after bootstrap thresholds are met — it becomes a regular instance that happened to come first.
...
**Purpose:** Define the protocol floor as structural prohibitions grounded in legal liability and protocol integrity. Establish instance-level authority as the correct layer for community values and content standards. Document why the floor is narrow and why that narrowness is a feature.

**Scope:**
- Protocol floor contains exactly two prohibitions, both grounded in real-world legal liability for real people: sexual content depicting real, identifiable minors (mandatory referral trigger); real, identifiable persons in sexual or defamatory context without consent (defamation and privacy liability)
- Everything not explicitly prohibited is unrestricted at the protocol level — the protocol does not enumerate allowed content
- Scanning infrastructure prohibited as a fixed architectural constraint, not a content policy position
- Instance-level content standards: operators may add standards above the floor; must publish and apply them uniformly; MUST NOT apply restrictions below the floor
- Photographic content moved to Section 14 as a scope statement — different legal and technical infrastructure requirements, not a content policy judgment
- AI-primary content moved to instance-level community standards — not a protocol floor concern; see Appendix A

**Design rationale — why the floor is narrow:** The original floor attempted to enumerate both structural prohibitions and community values statements in the same section. Pressure-testing revealed these have different natures and different stability. Structural prohibitions — those grounded in legal liability for the protocol infrastructure — are stable, legally coherent, and enforceable at the protocol level. Values statements — those expressing what this community believes about fiction, creative labor, and acceptable content — are not structurally enforceable at the protocol level, vary legitimately across communities, and resist definition with sufficient precision to function as binding rules. Conflating them produced unanswerable definition problems, authority capture risk, and logical inconsistencies in the protocol's own reasoning. The floor is narrowed to what it can actually deliver.

**Design rationale — fictional content policy belongs at instance layer:** The protocol's own reasoning for allowing gore — "fiction is not endorsement" — applies uniformly across content types. A protocol that treats fiction's relationship to reality as not its concern for violence cannot coherently claim fiction's relationship to reality IS its concern for other content types without a structural argument for the asymmetry. The structural argument does not exist for fictional content generally. The CSAM referral trigger is anchored to real, identifiable minors — the harm framework is about real people, not fictional characters. Fictional content policy, including community standards about fictional depictions of any kind, is correctly located at the instance layer where it can reflect community norms without imposing a single standard across all communities using this protocol.

**Design rationale — photographic content as scope statement:** Photographic content of real humans requires different infrastructure — rights management, real identity verification, DMCA takedown processes — that this protocol is not designed to provide. Moving this to Section 14 as a scope statement is more accurate than treating it as a content prohibition.

**Design rationale — no scanning infrastructure:** The E2EE architecture makes scanning architecturally impossible at the protocol level without breaking encryption. This is stated as a fixed architectural constraint in Section 11.3. The absence of scanning infrastructure is the structural defense against legislative backdoor mandates — [EARN IT Act](https://www.eff.org/deeplinks/2022/02/earn-it-act-back-and-its-still-unconstitutional), [Chat Control](https://www.patrick-breyer.de/en/posts/chat-control/), [Online Safety Act](https://www.eff.org/pages/uk-online-safety-bill-massive-threat-online-privacy-security-and-speech) — because infrastructure that does not exist cannot be mandated or expanded.
...
- Tiered determination by violation category:
  - Protocol floor violations (real person content without consent): creator response window → instance operator determination → creator appeal to governance process → deletion only after appeal exhausted or waived; sunset window protection applies during appeal
  - Sexual content depicting real, identifiable minors: immediate access suspension + mandatory referral to NCMEC CyberTipline (US), IWF (UK), and the INHOPE network (international); creator notified of suspension and referral immediately; the protocol conducts no independent adjudication; suspension is time-bounded — automatic reinstatement after a governance-parameter period (suggested 30 days) unless law enforcement has issued a preservation order or opened an active investigation, in which case suspension continues until that process concludes; deletion only after legal process finds claim substantiated; suspension lifted and outcome recorded on-chain if claim found unsubstantiated
  - Above-floor violations (instance-level content standards): governed entirely by instance operator following Section 7.5 removal procedures; creator appeal to governance available only where the creator claims the instance applied standards in violation of Section 11.4
...
The resumability and blob replacement timing requirements added to Section 4.7 apply to the on-demand re-encryption model adopted here — they are implementation correctness requirements, not compensating mechanisms for a rejected design.
...
This rejection applies specifically to protocol-held recovery infrastructure. It does not apply to participant-held emergency wallets, which are a distinct mechanism: the protocol holds nothing; the participant registers their own second wallet as an additional authorized key; the instance stores one master secret blob encrypted to multiple participant-held public keys. No new compellable target exists at the protocol layer. See emergency wallet design rationale in Section 2.

**EVM as canonical identity chain — decided**
Base (EVM L2) is the canonical identity chain. Identity contracts are deployed on Base. All participants require an EVM wallet for identity contract operations regardless of which payment rail they transact on. TRON and Solana are payment rails; they are not identity rails.
Rationale: EVM smart contract tooling is the most mature available by a significant margin; Base is the chosen primary L2 for its low fees, EVM-native architecture, and established infrastructure; the identity contract proxy pattern is well-validated on EVM through projects like ENS and Lens Protocol; stating this explicitly prevents inconsistent cross-chain identity assumptions across client and instance implementations.
Implication: a participant who transacts exclusively on Solana or TRON must still maintain an EVM wallet for identity operations. This is a real friction point for non-EVM-native participants and is stated honestly rather than obscured. The protocol is EVM-first in V1. Genuine multi-chain identity parity is a V2 scope item, requiring cross-chain state resolution in a trustless way that does not have a clean answer at this protocol's current scope.
...
The participant holds the upgrade key. No protocol authority holds upgrade keys over any participant's identity contract. The protocol publishes reference implementation addresses. Participants upgrade their contract logic at will by pointing their proxy to a new implementation.
Alternatives considered: immutable contract (cannot be patched on vulnerability discovery); globally upgradeable contract (reintroduces central authority as kill switch at the infrastructure layer). The proxy pattern with participant-held upgrade key is the correct middle position — each participant can upgrade their own contract; no participant can be upgraded without their consent; no central authority exists.
Non-upgraded participants run older logic indefinitely. This is accepted as the cost of decentralization. On critical vulnerability discovery, the governance process communicates urgency, the protocol publishes a new reference implementation, and participants upgrade at their own discretion. The migration window for critical vulnerabilities is therefore long, and this is the honest tradeoff of the chosen model.

**Emergency wallet as participant-held mitigation — decided**
The emergency wallet is not recovery infrastructure. It is an extension of the participant's own custody model: a second wallet they hold, registered as an additional authorized key against their identity contract, with the master secret blob re-encrypted to its public key.
The key distinction from protocol-held recovery: the protocol stores nothing additional; the participant controls all key material; no new compellable target exists at the protocol layer. An entity compelling the protocol learns nothing about the emergency wallet. An entity compelling the participant directly is a different threat model that the protocol cannot defend against by definition — holding the private key is being the identity.
Emergency wallet cold storage is load-bearing. An emergency wallet in daily use or with a visible transaction history is a linkable, potentially targetable second factor. An emergency wallet in cold storage with no prior history is a meaningful security improvement. The recommendation is not cosmetic.
Registration is optional. Participants who do not register an emergency wallet accept the full loss risk of single-wallet custody. This is stated honestly at onboarding. The protocol makes the mitigation available and easy; it cannot make it mandatory without creating its own problems (what if you lose both wallets?), and attempting to do so would contradict the participant-sovereignty principle.

**ZK pseudonymous rotation deliberately deferred to V2**
Zero-knowledge proof of identity continuity — proving that a new wallet address is the legitimate continuation of an old identity without publishing the linkage on-chain — is technically feasible using existing ZK proof systems. It is deferred from V1 for two reasons.
First, the ZK tooling ecosystem is not yet mature enough for V1 implementation without adding significant maintenance burden for a speculative benefit. Proof generation latency, auditing standards for ZK circuits, and client-side tooling support all require further maturation.
Second, the proxy contract model is designed to accommodate this upgrade. The participant-held upgrade key means that when ZK rotation support is added to a new reference implementation, participants can upgrade their identity contracts to support it without any protocol-level change and without migrating their identity contract address.
The V1 position — on-chain rotation linkage visible to chain analysts — is stated honestly in Section 2.6 rather than obscured. Participants who require stronger rotation privacy should use operational security practices at the wallet level (using a new wallet with no prior history for the new primary) rather than relying on protocol-layer privacy guarantees not yet available.
...
**Protocol floor restructure — values statements removed, structural prohibitions retained**
The original protocol floor in Section 11 enumerated both structural prohibitions grounded in legal liability and community values statements about acceptable fiction. These were treated as the same category of rule. Pressure-testing by the community revealed they have different natures: structural prohibitions are stable, legally coherent, and enforceable at the protocol level; values statements resist definition with sufficient precision to function as binding rules, vary legitimately across communities, and create authority capture risk when placed at the governance level.

Three specific problems drove the restructure. First, the fictional minor definition problem: no written rule can resolve the full vector space of fictional characters — species body plans, anthropomorphization spectrums, canonical developmental coding, and art style interact in ways that make any definitional boundary gameable or arbitrary. Every community test case broke the proposed rule at a different point. Second, authority capture: whoever holds gray zone determination authority effectively defines community values at the protocol level over time, which is opinion-gated governance by another name. Third, logical inconsistency: the protocol's own reasoning for allowing gore — "fiction is not endorsement" — applies uniformly across content types and cannot be asymmetrically applied to fictional minor content without a structural argument the protocol does not have.

The floor was narrowed to two structural prohibitions, both anchored in real-world legal liability for real people: sexual content depicting real, identifiable minors (CSAM referral trigger); real, identifiable persons in sexual or defamatory context without documented consent (defamation and privacy liability). Everything else is instance-level community standards. The protocol's identity — human creative labor, furry community focus, opposition to AI displacement of artists — is correctly expressed in the manifesto and implemented through instance standards, not enforced through protocol-level rules that cannot be made precise.

**AI-primary content prohibition — moved to instance-level community standards**
Originally prohibited at the protocol floor on the grounds that the protocol exists to sustain human creative labor. Moved to instance-level community standards for two reasons. First, the protocol cannot technically enforce it under E2EE — the same architectural reality applies to all content policy, but the AI prohibition has no structural legal liability grounding that would justify protocol-level treatment despite unenforceability. Second, the community itself enforces this norm — no instance serving the furry community would permit AI-primary content because the community treats it as equivalent to traced art, and the norm is strong enough to not require protocol enforcement. The automation abuse concern — bot accounts generating content at scale to game trust tiers or flood instances — is a protocol integrity concern handled by existing trust tier mechanics and self-transaction exclusion, not a content policy question. The manifesto's statement that DEN exists to sustain human creative labor remains the correct expression of this value; it does not need to be a binding protocol requirement to be real.

**Per-creator accounting for hoster compensation — rejected**
Considered as a mechanism to eliminate instance-size centralization incentive by calculating each creator's escrow and hoster claim independently, with compute overhead allocated per-creator rather than aggregated at the instance level. Rejected because it introduces significant smart contract complexity for a centralization signal (surplus return differential between instance sizes) that is empirically small — the actual per-creator advantage of being on a large instance is dominated by fixed compute overhead, which at protocol-set rates is a minor factor compared to content policy fit, operator reliability, and community. The per-creator escrow model is retained for the correct structural reason (no central treasury) but compensation uses the aggregate resource formula per instance per settlement interval. Accepted as a known approximation.

**Cap multiplier model for hoster surplus — rejected**
Considered as a mechanism where hosters keep up to N× their infrastructure cost and surplus above the cap returns to creators. Rejected because the cap introduces an arbitrary number governance can drift upward under operator pressure, and because the underlying problem — unbounded hoster surplus at large scale — is better solved by calibrating rates correctly than by adding a cap to a miscalibrated formula. Model B (hoster claims resource formula amount, surplus returns to creator) with properly calibrated above-market rates resolves the problem without introducing an arbitrary multiplier threshold.

**Labor component in hoster compensation formula — rejected**
Considered as an explicit maintenance labor line item in the resource formula — `hoster_fee = (storage × rate) + (bandwidth × rate) + (hours × labor_rate)`. Rejected for three reasons: labor hours are not verifiable on-chain, creating an undetectable overclaim vector; an explicit labor rate is an inherently political governance parameter that invites ideological disagreement unrelated to protocol health; and the problem it solves — ensuring hosters earn above break-even — is correctly solved by calibrating storage and bandwidth rates above market cost, embedding the margin in the rate rather than naming it separately. The margin is real; naming it as labor is the problem.
...
**Creator oracle / threshold key split — removed from V1**
An earlier version of the key delivery architecture used a two-party XOR threshold split. The master secret was split into `instance_share` (stored by the instance as the operational blob) and `creator_share` (held by an independently-operated per-creator oracle service). Key delivery required cooperation from both parties: the instance decrypted its half, the oracle returned the creator's half, and `master_secret = instance_share XOR creator_share` was reconstructed transiently.

The design goal was to eliminate the single-party capture vector: a compelled instance hoster would obtain only `instance_share`, which is information-theoretically indistinguishable from random noise without the corresponding `creator_share`.

Removed from V1 for three compounding reasons:

*The separation cannot be cryptographically enforced.* The oracle model relies on organisational and legal independence between the instance operator and the oracle operator. Nothing in the protocol prevents the same entity from running both. A hoster who also runs the oracle — which is the rational default behaviour, since the alternative requires creators to operate and maintain their own server infrastructure — trivially collapses the "two-party" split into one. There is no mechanism by which any other party can detect or prove this has happened, because the oracle is an off-chain HTTP service with no on-chain accountability.

*Creator liveness becomes a service dependency.* If a creator's oracle is offline, their subscribers cannot access any content — regardless of whether they have valid on-chain subscriptions. A jurisdiction-ordered takedown of the instance achieves its goal by targeting the oracle instead. The defence against legal compulsion requires both the instance and the oracle to be independently resilient; they cannot both be required to be online simultaneously. The cure is worse than the disease: the oracle introduces a second single point of failure rather than eliminating the first one.

*The problem being solved is jurisdictional availability, not key custodianship.* Legal compulsion is a jurisdictional and availability problem. The actual defences available are: (a) E2EE at rest so the hoster stores only ciphertext in normal operation — this holds regardless of whether a threshold split exists; (b) fast portability so a creator can migrate to a different-jurisdiction instance within hours of a compulsion attempt; (c) key rotation so the seized instance's copy of the operational blob becomes useless for new content immediately after migration. These guarantees hold independent of the oracle model. Adding an organisational split between two services operated in the same jurisdiction under the same legal pressure achieves nothing.

The V1 security model is: the instance stores ciphertext only. E2EE at rest is the primary operational guarantee. Portability and key rotation are the primary defences against compelled disclosure. This is the same model as Signal's server-side message storage and every comparable E2EE-at-rest architecture.

V2 path: if threshold decryption backed by a decentralised independent custodian network becomes production-ready — where access policies are enforced cryptographically rather than operationally, and no single operator can be compelled to collapse the split — the threshold model should be revisited. The file `src/oracle/client.ts` is retained in the V1 implementation as a documented starting point for this work. See Section 4.1 design note in the spec.
